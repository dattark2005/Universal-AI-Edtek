...
import { getAssignments, getSubmissions, saveSubmission, getCurrentUser, getUserClassrooms } from '../../utils/storage';
...
const AssignmentsSection: React.FC = () => {
  ...
  const assignments = [...getAssignments(), ...mockAssignments];
  const submissions = getSubmissions();
  const userClassrooms = user ? getUserClassrooms(user.id, 'student') : [];
  
  // Filter assignments for classrooms the student is in
  const studentAssignments = assignments.filter(assignment => {
    if (assignment.classroomId) {
      return userClassrooms.some(classroom => classroom.id === assignment.classroomId);
    }
    return true; // Show mock assignments for demo
  });
  
  // Get user's submissions
  const userSubmissions = user ? submissions.filter(sub => sub.studentId === user.id) : [];
        ...
        {userClassrooms.length === 0 && (
          <div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-4 max-w-md mx-auto">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-blue-300 font-medium">Join a Classroom</span>
            </div>
            <p className="text-white/70 text-sm">
              Go to the Classrooms section and use a class code to join your teacher's classroom.
            </p>
          </div>
        )}
      </div>
    ...
    <div>
      ...
      <div className="space-y-6">
        {studentAssignments.map((assignment) => {
          ...
          return (
            <div key={assignment.id} className="bg-gradient-to-r from-indigo-500/20 to-purple-500/20 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-semibold text-white mb-2">{assignment.title}</h3>
                  <p className="text-white/70 mb-2">By {assignment.teacherName}</p>
                  ...
                  {isSubmitted && submission && (
                    <div className="bg-green-500/20 text-green-300 px-3 py-1 rounded-full text-sm font-medium">
                      ...
                      {submission.grade !== undefined && (
                        <span className="ml-2">• Grade: {submission.grade}%</span>
                      )}
                    </div>
                  )}
                </div>
